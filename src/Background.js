...
class Background {
  constructor() {
    ...
    for (let i = 0; i < Config.background.numStars * 3; i++) {
      starVerts.push(Math.random() * 600 - 300,)
    }
    starVerts = new Float32Array(starVerts);
    this.starGeo.setAttribute( 'position', 
      new THREE.BufferAttribute(starVerts, 3).setUsage(THREE.DynamicDrawUsage));
  }
  ...
  update() {
    ...
    for (let i = 2, j = 0; i < len; i += 3, j++) {
      ...
      this.stars.geometry.getAttribute("position").array[i] += this.stars.userData[j];
  ...
  loadTexture() {
    return new Promise((resolve, reject) => {
      new THREE.TextureLoader().load(Config.background.starPath, 
